package com.example.bookspace.view_model.category;
...
public class CategoryDetailsViewModel extends ViewModel {
    ...
    private MediatorLiveData<Resource<List<BookDetails>>> bookList = new MediatorLiveData<>();

    public CategoryDetailsViewModel(ApiInterface apiInterface) {
        this.apiInterface = apiInterface;
    }

    public LiveData<Resource<List<BookDetails>>> observeBookList(String category) {
        bookList.setValue(Resource.loading(null));
        LiveData<Resource<List<BookDetails>>> source = LiveDataReactiveStreams.fromPublisher(
                apiInterface.getAllBookByCategory(category)
