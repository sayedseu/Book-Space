...
public class ExchangeWithCashAdapter extends RecyclerView.Adapter<ExchangeWithCashAdapter.ExchangeWithCashViewHolder> {
    private List<BookDetails> bookDetailsList;
    private SharedViewModel viewModel;
    private NavController navController;

    public ExchangeWithCashAdapter(SharedViewModel viewModel, NavController navController) {
    ...
    @NonNull
    @Override
    public ExchangeWithCashViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.single_book_card, parent, false);
        return new ExchangeWithCashViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ExchangeWithCashViewHolder holder, int position) {
        if (bookDetailsList != null) {
            BookInfo bookInfo = bookDetailsList.get(position).getBookInfo();
            ExpectedBook expectedBook = bookDetailsList.get(position).getExpectedBook();
            ...
            holder.bookPrice.setText("\u09F3 " + expectedBook.getBookPrice());
    ...
    static class ExchangeWithCashViewHolder extends RecyclerView.ViewHolder {
        ...
        ExchangeWithCashViewHolder(@NonNull View itemView) {
            super(itemView);
